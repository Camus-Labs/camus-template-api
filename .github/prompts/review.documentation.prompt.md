...
**Failure:** Any input validation in Steps 1–2 fails, or Step 3 discovers zero documentation files.
...
2. Read every file in `modified_files` using the `codebase` tool — understand public types, interfaces,
  configuration keys, settings structures, endpoints, and behavioral contracts; if any file is unreadable, stop and
  report the problem; otherwise proceed to Step 3.

3. Discover documentation files using the `codebase` tool — for each modified file, locate its containing project
  directory (the folder with the `.csproj` file) and collect the `README.md` in that directory if it exists; otherwise
  skip that project directory; then collect `README.md` and `docs/README.md` in the workspace root as baseline files,
  plus every other `docs/*.md` file that references types, configuration keys, settings, or features found in Step 2;
  deduplicate the resulting list; if the search finds zero documentation files, stop and report the problem; otherwise
  proceed to Step 4.

4. Read every discovered documentation file using the `codebase` tool — if any file is unreadable, stop and report
  the problem; otherwise proceed to Step 5.

5. Evaluate content coherence — for each documentation file, verify every technical claim, configuration reference,
  type name, endpoint description, and behavioral statement against the source code from Step 2; record a finding
  when documentation describes something that contradicts, misrepresents, or no longer matches the source code;
...
6. Evaluate each documentation conventions checklist section (Information Ownership, Single Source of Truth,
  Cross-Reference Integrity, Content Accuracy, Structure & Formatting) against the documentation files using the
  `codebase` tool — score `PASS` only when ALL checklist items in that section pass for ALL documentation files,
  otherwise score `FAIL`, and record each failing item as a finding; proceed to Step 7.

7. Compute the overall verdict — PASS when every section including Content Coherence is PASS, otherwise FAIL — use
  no tools.

8. Produce the report in the exact output format below using the verdict and all findings — use no tools.
...
- MUST use section names exactly as listed: "Content Coherence" followed by the conventions checklist section names
