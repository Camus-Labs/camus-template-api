...
3. Validation & Error Handling
    ...
    - [ ] Validation lives in mapper extensions — controllers call `ToCommand()` / `ToFilter()` / `ToPaginationParams()`
      / `ToFilter()` / `ToResponse()` / `ToDto()` and let mappers validate before the application layer is reached
    - [ ] Mappers use guard inputs with `ArgumentException.ThrowIf*()` static helpers
