...
tools: ['codebase', 'editFiles', 'terminal', 'createFile', 'createDirectory']
---
...
Create a TDD-ready red-phase package — production stubs scaffolded from the Layer Impact Matrix (Section B) and a
matching test suite covering every acceptance criterion (Section A). The developer's only remaining job is to fill in
the production logic until all tests pass.
...
3. Run `dotnet build src/CamusApp.sln` via the `terminal` tool to verify the production skeleton compiles — if the
  build fails, fix compilation errors in stub files using `editFiles` and re-run up to 5 times; if the build still
  fails after 5 attempts, stop and report the compilation errors; otherwise proceed to Step 4.

4. Present the production skeleton to the user for review — list every stub file created, its layer, and a summary of
  types and members; if the user requests changes, apply them using `editFiles`, rebuild via `terminal`, and re-present
  up to 5 cycles; if the user does not approve after 5 cycles, stop and report outstanding objections; proceed to
...
6. Verify tests compile — run `dotnet build src/CamusApp.sln` via the `terminal` tool; if the build fails, fix
  compilation errors in test files using `editFiles` and re-run up to 5 times; if the build still fails after
  5 attempts, stop and report the compilation errors; otherwise proceed to Step 7.

7. Verify tests fail for the right reason — run `dotnet test src/CamusApp.sln --no-build` via the `terminal` tool; if
  all new tests fail as expected, proceed to Step 8; if any new test passes (stub accidentally satisfies it), redesign
  that test to assert a meaningful behavior the stub cannot satisfy using `editFiles`, rebuild, re-run, and repeat up
  to 5 times; if the test still passes after 5 redesign attempts, stop and report the test as unresolvable; otherwise
  proceed to Step 8.

8. Populate Section C in the story file using `editFiles`: fill the Skeleton Inventory table with every stub file
  created (layer, file path, types, members), fill the Test Traceability table with every test method created (AC,
  test class, test method, layer), evaluate and set each Tester Handoff Gate item, set tester sign-off — include the
  Tester Handoff Report block shown in Output Format as the final section of the populated output — and stop.
