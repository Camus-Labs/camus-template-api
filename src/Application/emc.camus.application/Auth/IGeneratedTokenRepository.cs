...
public interface IGeneratedTokenRepository
{
    ...
    Task CreateAsync(IDbConnection connection, GeneratedToken generatedToken);
    ...
    Task<GeneratedToken?> GetByJtiAsync(IDbConnection connection, Guid jti);
    ...
    Task<PagedResult<GeneratedToken>> GetPagedByCreatorUserIdAsync(IDbConnection connection, Guid creatorUserId, PaginationParams pagination, GeneratedTokenFilter? filter = null);
    ...
    /// <returns>A task representing the asynchronous operation.</returns>
    Task SaveAsync(IDbConnection connection, GeneratedToken generatedToken);
}
